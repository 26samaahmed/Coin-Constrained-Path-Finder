# updated gui.py with calculations
...
class PathFinderGUI:
    ...
    def create_widgets(self):
        ...
        style = ttk.Style()
        style.theme_use('clam')  # clam theme for better customization
        ...
        self.result_text = tk.Text(text_container, font=("Almendra", 20),
                                   bg="#EBD4CB", fg="#2C0603", 
    ...
    def show_ai_solution(self):
        ...
        for i, city in enumerate(ai_path):
            ...
            if i == 0:
                # Starting node
                f = 0 + h
                comparison_text += f"{city.cityName}:\n"
                comparison_text += f"  g = 0 (start)\n"
                comparison_text += f"  h = {h}\n"
                comparison_text += f"  f = 0 + {h} = {f}\n\n"
            else:
                ...
                comparison_text += f"{prev_city.cityName} → {city.cityName}:\n"
                comparison_text += f"  Edge: distance={edge_dist}, coins={edge_coins}\n"
                comparison_text += f"  g = {prev_g} + {edge_dist} = {cumulative_g}\n"
                comparison_text += f"  h = {h}\n"
                comparison_text += f"  f = {cumulative_g} + {h} = {f}\n\n"
